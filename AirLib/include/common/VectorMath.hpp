...
#if _FOR_LIB_PACKAGE_

#else
            Pose(const FVector& position_val, const FQuat& orientation_val)
            {
                orientation = QuaternionT(orientation_val.W, orientation_val.X, orientation_val.Y, orientation_val.Z);
                position = Vector3T(position_val.X,position_val.Y,position_val.Z);
            }

            Pose(const FTransform& pose)
            {
                orientation = QuaternionT(pose.GetRotation().W,-pose.GetRotation().X,-pose.GetRotation().Y,pose.GetRotation().Z);
                position = Vector3T(0.01f*pose.GetLocation().X,0.01f*pose.GetLocation().Y,-0.01f*pose.GetLocation().Z);
            }
