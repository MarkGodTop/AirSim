...
public:
	...
	{
		//TODO:@fixme->新增其他的变量（现控制及发动机设置逻辑与JSBSimMovementComponent中保持一致）
		// based on ardupilot definition [a, e, t, r]
		double aileron = 0.0f;
		double elevator = 0.0f;
...
public:
	...
	void addSensorsFromSettings(const AirSimSettings::VehicleSetting* vehicle_setting)
	{
		...
		const std::map<std::string, std::shared_ptr<AirSimSettings::SensorSetting>>& sensor_settings
			= vehicle_setting->sensors.size() > 0 ? vehicle_setting->sensors : AirSimSettings::singleton().sensor_defaults;
		sensor_factory_->createSensorsFromSettings(sensor_settings, sensors_, sensor_storage_);
	}
