...
namespace airlib
{
    ...
    public:
         ...
         */
        virtual void updateLocalPositionData(const float locX, const float locY, const float locZ, const int64_t timeStamp, const int opFlag) = 0;
        virtual void updateLocalRotationData(const float quatW, const float quatX, const float quatY, const float quatZ, const int64_t timeStamp) = 0;
        virtual void updateLocalDefaultVelocityData(const float velocity) = 0;
        virtual void updateLocalDetectTargetNumData(const std::string jsonData) = 0;
        virtual void updateLocalTargetDistanceData(const float distance, const float targetPosX, const float targetPosY, const float targetPosZ, const int targetType, const bool bIsDynamic) = 0;
        virtual void updateLocalDetectTargetPreData(const float targetPosX, const float targetPosY, const float targetPosZ, const int targetType) = 0;
