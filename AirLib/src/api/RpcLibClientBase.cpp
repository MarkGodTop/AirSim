    ...
    namespace msr
{
    namespace airlib
    {
        ...
        void RpcLibClientBase::simUpdateLocalPositionData(const std::string vehicle_name, float locX, float locY,
            float locZ, int64_t timeStamp, int opFlag, std::string pointName)
        {
            pimpl_->client.call("simUpdateLocalPositionData",vehicle_name,locX,locY,locZ,timeStamp,opFlag,pointName);
        }

        void RpcLibClientBase::simUpdateLocalRotationData(const std::string vehicle_name, float quatW, float quatX,
            float quatY, float quatZ, int64_t timeStamp, std::string pointName)
        {
            pimpl_->client.call("simUpdateLocalRotationData",vehicle_name,quatW,quatX,quatY,quatZ,timeStamp,pointName);
        }

        void RpcLibClientBase::simUpdateLocalDefaultVelocityData(const std::string vehicle_name, float velocity, std::string pointName)
        {
            pimpl_->client.call("simUpdateLocalDefaultVelocityData",vehicle_name,velocity,pointName);
        }

        void RpcLibClientBase::simUpdateLocalDetectTargetNumData(const std::string vehicle_name, const std::string jsonData, std::string pointName)
        {
            pimpl_->client.call("simUpdateLocalDetectTargetNumData",vehicle_name,jsonData,pointName);
        }

        void RpcLibClientBase::simUpdateLocalTargetDistanceData(const std::string vehicle_name, float targetDistance,
            float targetPosX, float targetPosY, float targetPosZ, int targetType, bool bIsDynamic, std::string pointName)
        {
            pimpl_->client.call("simUpdateLocalTargetDistanceData",vehicle_name,targetDistance,targetPosX,targetPosY,targetPosZ,targetType,bIsDynamic,pointName);
        }

        void RpcLibClientBase::simUpdateLocalDetectTargetPreData(const std::string vehicle_name, float targetPosX,
            float targetPosY, float targetPosZ, int targetType, std::string pointName)
        {
            pimpl_->client.call("simUpdateLocalDetectTargetPreData",vehicle_name,targetPosX,targetPosY,targetPosZ,targetType,pointName);
        }

        void RpcLibClientBase::simUpdateLocalCheckNaviData(const std::string vehicle_name, bool bIsInNoiseAreaByRos,
            std::string pointName)
        {
            pimpl_->client.call("simUpdateLocalCheckNaviData",vehicle_name,bIsInNoiseAreaByRos,pointName);
        }
