...
void AirsimROSWrapper::drone_state_timer_cb(const ros::TimerEvent& event)
{
    try {
        ...
        if (now < ros::Time(0)) {
            // Skip publishing and log an error
            ROS_ERROR("Negative timestamp detected. Skipping publishing.");
            return;
        }
...
{
    std::set<ros::Time> published_timestamps; // 用于存储已发布的时间戳
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        auto& vehicle_ros = vehicle_name_ptr_pair.second;
        const ros::Time& timestamp = vehicle_ros->env_msg.header.stamp;
        if (timestamp.toSec() < 0)
            continue;
        if (published_timestamps.count(timestamp) > 0) {
            continue; // 如果时间戳已经发布过，则跳过
        }
        ...
        published_timestamps.insert(timestamp); // 记录已发布的时间戳
        // odom and transforms
        // 保存上一个时间戳
        static ros::Time previous_time;
        // 获取当前时间戳
        ros::Time current_time = vehicle_ros->curr_odom.header.stamp;
        // 如果当前时间戳与上一个时间戳不同，则进行发布
        if (current_time != previous_time && current_time > previous_time) {
            // 发布里程计数据
        ...
        for (auto& sensor_publisher : vehicle_ros->sensor_pubs) {
            ...
            case SensorBase::SensorType::Barometer: {
                ...
                if(alt_msg.header.stamp.toSec() < 0 || alt_msg.header.stamp == previous_time1)
                    break;
                sensor_publisher.publisher.publish(alt_msg);
                previous_time1 = alt_msg.header.stamp;
                break;
            }
            case SensorBase::SensorType::Imu: {
                ...
                static geometry_msgs::Vector3 last_acceleration;
                if (imu_msg.linear_acceleration != last_acceleration) {
                    if(imu_msg.header.stamp.toSec() < 0)
                        break;
                    sensor_publisher.publisher.publish(imu_msg);
                    last_acceleration = imu_msg.linear_acceleration;
                }
                break;
            }
            case SensorBase::SensorType::Distance: {
                ...
                static ros::Time previous_time2;
                if(dist_msg.header.stamp.toSec() < 0 || dist_msg.header.stamp == previous_time2)
                    break;
                previous_time2 = dist_msg.header.stamp;
            ...
            case SensorBase::SensorType::Gps: {
                ...
                static ros::Time previous_time3;
                if(gps_msg.header.stamp.toSec() < 0 || gps_msg.header.stamp == previous_time3)
                    break;
                previous_time3 = gps_msg.header.stamp;
            ...
            case SensorBase::SensorType::Magnetometer: {
                ...
                static ros::Time previous_time4;
                if(mag_msg.header.stamp.toSec() < 0 || mag_msg.header.stamp == previous_time4)
                    break;
                sensor_publisher.publisher.publish(mag_msg);
                previous_time4 = mag_msg.header.stamp;
                break;
            }
            }
        }
